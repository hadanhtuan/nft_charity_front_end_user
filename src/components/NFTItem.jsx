...
function NFTItem({ title, price, remainingTime }) {
  ...
  return (
    <Card sx={{ width: '280px' }}>
      ...
      >
        <CardMedia component="img" image={img2} sx={{ height: '100%', width: '100%' }} />
      </Box>
      <Box>
        <Box>
          <Box sx={{ display: 'flex', padding: '8px', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Box sx={{ display: 'flex', padding: '8px' }}>
            <Typography variant="body1">
              Remaining time:
              {remainingTime ? <DateCountdown dateTo={endAt} callback={() => console.log('finished')} /> : 'No time'}
            </Typography>
          </Box>
        </Box>
      </Box>
